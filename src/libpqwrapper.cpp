...
PgResult::PgResult(PGresult* result) : result_(result, &PQclear)
{}
...
PgResult
    PgConnection::ExecWithParams(const std::string& sql,
                                 const std::vector<std::string>& params) const
{
    ...
    return PgResult(PQexecParams(connection_.get(), sql.c_str(),
                                 static_cast<int>(cParams.size()), nullptr,
