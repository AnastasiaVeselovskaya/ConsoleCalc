...
Cacher::Cacher() : connection_(dbConnectionStr)
{
    WarmUpCache();
}
...
std::variant<ErrorCode, double>
    Cacher::GetResultFromCache(int64_t first, char operation,
...
void Cacher::SaveToShortTermCache(const CalculationLog& record)
{
    ...
    std::string key = MakeCacheKey(
        record.leftNum, record.operation,
...
std::string Cacher::MakeCacheKey(int64_t first, char operation,
...
std::variant<ErrorCode, double>
    Cacher::GetResultFromDB(int64_t first, char operation,
                            std::optional<int64_t> second) const
{
    const std::string sql = R"(
    ...
    std::vector<std::string> params = {std::to_string(first),
